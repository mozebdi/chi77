    ...
    def handle(self, *args, **options):
        ...
        for neighborhood in Neighborhood.objects.all():
            ...
            for elt in zips:
                ...
                if ratio > 0.0001:
                    n2z = NeighborhoodToZip(geom=overlap, zip=elt.zip, neighborhood=neighborhood.id, pct=ratio)
                    n2z.save()
            for elt in tracts:
                overlap = filtershapes(elt.geom.intersection(neighborhood.geom))
                ratio = overlap.area / elt.geom.area
                if ratio > 0.0001:
                    n2t = NeighborhoodToTract(geom=overlap, tract=elt.name10, neighborhood=neighborhood.id, pct=ratio)
                    n2t.save()
            for elt in precincts:
                overlap = filtershapes(elt.geom.intersection(neighborhood.geom))
                ratio = overlap.area / elt.geom.area
                if ratio > 0.0001:
                    n2p = NeighborhoodToPrecinct(geom=overlap, precinct=elt.id, neighborhood=neighborhood.id, pct=ratio)
                    n2p.save()
            for elt in wards:
                overlap = filtershapes(elt.geom.intersection(neighborhood.geom))
                ratio = overlap.area / elt.geom.area
                if ratio > 0.0001:
                    n2w = NeighborhoodToWard(geom=overlap, ward=elt.ward, neighborhood=neighborhood.id, pct=ratio)
                    n2w.save()
        for zipcode in Zip.objects.all():
            ...
            for elt in tracts:
                ...
                ratio = overlap.area / elt.geom.area
                if ratio > 0.0001:
                    z2t = ZipToTract(geom=overlap, tract=elt.name10, zip=zipcode.zip, pct=ratio)
                    z2t.save()
            for elt in precincts:
                overlap = filtershapes(elt.geom.intersection(zipcode.geom))
                ratio = overlap.area / elt.geom.area
                if ratio > 0.0001:
                    z2p = ZipToPrecinct(geom=overlap, precinct=elt.id, zip=zipcode.zip, pct=ratio)
                    z2p.save()
            for elt in wards:
                overlap = filtershapes(elt.geom.intersection(zipcode.geom))
                ratio = overlap.area / elt.geom.area
                if ratio > 0.0001:
                    z2w = ZipToWard(geom=overlap, ward=elt.ward, zip=zipcode.zip, pct=ratio)
                    z2w.save()
        for ward in Ward.objects.all():
            ...
            for elt in tracts:
                ...
                ratio = overlap.area / elt.geom.area
                if ratio > 0.0001:
                    w2t = WardToTract(geom=overlap, tract=elt.name10, ward=ward.ward, pct=ratio)
                    w2t.save()
        for tract in Tract.objects.all():
            ...
            for elt in precincts:
                ...
                ratio = overlap.area / elt.geom.area
                if ratio > 0.0001:
                    t2p = TractToPrecinct(geom=overlap, precinct=elt.id, tract=tract.name10, pct=ratio)
                    t2p.save()
